...
def pdf_to_text(pdf_bytes):
    """Extract text from PDF — first try native text, then OCR"""
...
def parse_fields(text, doc_type="parent"):
    """Parse fields from extracted text using regex patterns"""
    result = {}
    ...
    if doc_type == "parent":
        ...
        if iin_match:
            result['parent_iin'] = iin_match.group(1)

        # Document number — typically 9 digits for KZ ID
        doc_match = re.search(r'(?:№|N|No|уд[^\d]*|серия[^\d]*|лич[^\d]*)\s*([A-Z]{0,2}\d{7,9})', text, re.IGNORECASE)
        if doc_match:
            result['parent_doc_num'] = doc_match.group(1)

        # Date issued — ДД.ММ.ГГГГ or ДД/ММ/ГГГГ
        dates = re.findall(r'\b(\d{2}[./]\d{2}[./]\d{4})\b', text)
        if dates:
            result['parent_doc_date'] = dates[0].replace('/', '.')

        # FIO — try to find Cyrillic name (3 capitalized words)
        fio_match = re.search(r'([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})', text)
        if fio_match:
            result['parent_fio'] = fio_match.group(1)

        # Address
        addr_match = re.search(r'(?:адрес|прож|зарег)[^\n:]*[:\s]+([^\n]{10,80})', text, re.IGNORECASE)
        if addr_match:
        ...
        iin_match = re.search(r'\b(\d{12})\b', text)
        if iin_match:
            result['child_iin'] = iin_match.group(1)

        # FIO
        fio_match = re.search(r'([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,2})', text)
        if fio_match:
            result['child_fio'] = fio_match.group(1)

    return result


def extract_from_pdfs(parent_bytes, child_bytes):
    """Extract all needed fields from both PDFs"""
    result = {}

    try:
        parent_text = pdf_to_text(parent_bytes)
        logger.info(f"Parent PDF text (first 300): {parent_text[:300]}")
        parent_fields = parse_fields(parent_text, "parent")
        result.update(parent_fields)
        result['_parent_text'] = parent_text  # save for manual check
    except Exception as e:
        logger.error(f"Parent PDF error: {e}")

    try:
        child_text = pdf_to_text(child_bytes)
        logger.info(f"Child PDF text (first 300): {child_text[:300]}")
        child_fields = parse_fields(child_text, "child")
        result.update(child_fields)
        result['_child_text'] = child_text
    except Exception as e:

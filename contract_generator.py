...
def generate_contract(data, template_path):
    ...
    # === P2: Date line ===
    # run9='  '(day+space), run12='___'(month), run13=' 202', run14='6'(year_last)
    if cd:
        p2 = paras[2]
        r = p2.runs
        if len(r) > 14:
    ...
    # === P4: Parent data ===
    # run16=parent_fio, run18=parent_iin+', ', run19=doc_num, run21=day_issued, run23=month_issued, run27=address
    p4 = paras[4]
    r4 = p4.runs
    if len(r4) > 27:
        r4[16].text = pfio + ','
        r4[17].text = '\nИИН '
        r4[18].text = piin + ', '
        r4[19].text = 'удостоверение личности № ' + pdoc
        r4[20].text = ''
        r4[21].text = ', выдано «'
        if pdd:
            r4[22].text = pdd['day'] + '»'
            r4[23].text = ' ' + pdd['month_num'] + ' 20' + pdd['year'][2:]
        else:
            r4[22].text = '»'
            r4[23].text = ' ____'
        r4[27].text = ': ' + paddr + ' ('
    ...
    # === P8: Period ===
    # Structure: '1.2...с «' run1='» ' run2='__________ 202' run3='6' run4=' ' run5='г. ' run6='по «' run7='__' run8='» ' run9='__________ ' run10='2026 г.'
    if dfd and dtd:
        p8 = paras[8]
        r8 = p8.runs
        if len(r8) > 10:
    ...
    # run0='от «____» _______ 202', run1='6', run2=' года'
    if cd:
        p153 = paras[153]
        r153 = p153.runs
        if len(r153) > 1:
        ...
        # === P157: App1 city+date ===
        # run4='___» _______ 202', run5='6', run6=' года'
        p157 = paras[157]
        r157 = p157.runs
        if len(r157) > 5:
